...
trait ZipkinTraceServiceLike {
   ...
   */
  private[play] def serverReceived(spanName: String, span: Span): ServerSpan = {
   ...
   */
  private[play] def serverSend(span: ServerSpan, tags: (String, String)*): ServerSpan = {
   ...
   */
  private[play] def newSpan[A](headers: A)(getHeader: (A, String) => Option[String]): Span = {
   ...
   */
  private[play] def newSpan(parent: Option[TraceContext]): Span = {
    parent match {
   ...
   */
  private[play] def toSpan[A](headers: A)(getHeader: (A, String) => Option[String]): Span = {
    val contextOrFlags = tracing.propagation().extractor(
      (carrier: A, key: String) => getHeader(carrier, key).orNull
    ).extract(headers)
   ...
   */
  private[play] def toMap(span: Span): Map[String, String] = {
   ...
   */
  private[play] def toMap(traceData: TraceData): Map[String, String] = {
